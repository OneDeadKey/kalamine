...
def xkb_ensure_xkb_config_is_ready():
    """ Ensure the requested layout locale exists in user-space. """
    # Most of this function is taken from xkblayout.py:
    # https://gitlab.freedesktop.org/whot/xkblayout

    # First ensure all expected directories exist (don't care about 'geometry')
    os.makedirs(XKB_HOME, exist_ok=True)
    for d in ['compat', 'keycodes', 'rules', 'symbols', 'types']:
        os.makedirs(os.path.join(XKB_HOME, d), exist_ok=True)
...
def install(layouts):
    ...
    try:
        ...
        # xkb_ensure_locales_are_ready(kb_locales)
